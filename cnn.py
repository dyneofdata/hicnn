...
if mode == 'vanilla':
        ...
        model = Sequential()
        model.add(Conv1D(input_shape = shape, filters = 10, kernel_size = 4, strides = 1, padding = 'same', activation = 'relu', use_bias = True, kernel_initializer = 'glorot_uniform', bias_initializer = 'glorot_uniform'))
        model.add(MaxPooling1D(padding='same'))
        model.add(Dropout(0.5))
        model.add(Conv1D(filters = 10, kernel_size = 4, strides = 1, padding = 'same', activation = 'relu', use_bias = True, kernel_initializer = 'glorot_uniform', bias_initializer = 'glorot_uniform'))
        model.add(MaxPooling1D(padding='same'))
        model.add(Dropout(0.5))
        model.add(Flatten())
...
model.fit(X, Y, batch_size = 32, epochs = 100, verbose = 2, callbacks = [early_stopping, lrate, csv_logger, checkpoint], validation_split = 0.1, shuffle = True)
