...
class Model:
    ...
    def __init__(self, chromosome, cell):
        ...
        self.pool_size =[5,10,15]
        
        self.num_dense_layers = 3
        self.dropout_prob = [0.3, 0.3, 0.3]
    ...
    def train(self):
        ...
        pool_X1 = MaxPooling1D(pool_size = self.pool_size[0])(conv_X1)
        conv_X1 = Conv1D(filters = self.num_filters[1],
                             kernel_size = self.kernel_size[1],
                             strides = self.stride[1],
                             activation = 'elu')(pool_X1)
        pool_X1 = MaxPooling1D(pool_size = self.pool_size[1])(conv_X1)
        
        conv_X2 = Conv1D(filters = self.num_filters[0],
                             kernel_size = self.kernel_size[0],
                             strides = self.stride[0],
                             activation = 'elu')(input_X2)
        pool_X2 = MaxPooling1D(pool_size = self.pool_size[0])(conv_X2)
        conv_X2 = Conv1D(filters = self.num_filters[1],
                             kernel_size = self.kernel_size[1],
                             strides = self.stride[1],
                             activation = 'elu')(pool_X2)
        pool_X2 = MaxPooling1D(pool_size = self.pool_size[1])(conv_X2)

        flat_X1 = Flatten()(pool_X1)
        flat_X2 = Flatten()(pool_X2)

        dense = Concatenate()([flat_X1, flat_X2, input_X3])
        dense = Dropout(0.2)(dense)
        dense = Dense(self.hidden_dims[1], activation = 'elu')(dense)
